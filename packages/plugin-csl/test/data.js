const bibliographyData = [
  {
    id: 'Q23571040',
    type: 'article-journal',
    title: 'Correlation of the Base Strengths of Amines 1',
    DOI: '10.1021/ja01577a030',
    author: [
      {
        given: 'H. K.',
        family: 'Hall'
      }
    ],
    issued: { 'date-parts': [[1957, 1, 1]] },
    'container-title': 'Journal of the American Chemical Society',
    volume: '79',
    issue: '20',
    page: '5441-5444'
  }
]

const citationData = [{
  id: '1',
  type: 'article-journal',
  title: 'a',
  issued: { 'date-parts': [[2011]] }
}, {
  id: '2',
  type: 'article-journal',
  title: 'b',
  author: [{
    family: 'd',
    given: 'c'
  }, {
    literal: 'h'
  }],
  issued: { 'date-parts': [[2012]] }
}, {
  id: '3',
  type: 'article-journal',
  title: 'e',
  author: [{
    family: 'f',
    given: 'g'
  }],
  issued: { 'date-parts': [[2013]] }
}]

module.exports = {
  bibliography: {
    'default built-in style': [
      bibliographyData,
      'Hall, H. K. (1957). Correlation of the Base Strengths of Amines 1. Journal of the American Chemical Society, 79(20), 5441–5444. https://doi.org/10.1021/ja01577a030',
      { style: 'apa' }
    ],
    'non-default built-in style': [
      bibliographyData,
      '1. Hall HK. Correlation of the Base Strengths of Amines 1. Journal of the American Chemical Society. 1957 Jan 1;79(20):5441–4. doi:10.1021/ja01577a030',
      { style: 'vancouver' }
    ],
    'deprecated "template" option': [
      bibliographyData,
      '1. Hall HK. Correlation of the Base Strengths of Amines 1. Journal of the American Chemical Society. 1957 Jan 1;79(20):5441–4. doi:10.1021/ja01577a030',
      { template: 'vancouver' }
    ],
    'prioritizing "style" over "template" option': [
      bibliographyData,
      '1. Hall HK. Correlation of the Base Strengths of Amines 1. Journal of the American Chemical Society. 1957 Jan 1;79(20):5441–4. doi:10.1021/ja01577a030',
      { style: 'vancouver', template: 'chicago' }
    ],
    'unavailable dialect': [
      bibliographyData,
      'Hall, H. K. (1957). Correlation of the Base Strengths of Amines 1. Journal of the American Chemical Society, 79(20), 5441–5444. https://doi.org/10.1021/ja01577a030',
      { lang: 'en-GB' }
    ],
    'unavailable locale': [
      bibliographyData,
      'Hall, H. K. (1957). Correlation of the Base Strengths of Amines 1. Journal of the American Chemical Society, 79(20), 5441–5444. https://doi.org/10.1021/ja01577a030',
      { lang: 'pt-PT' }
    ],
    'non-existent locale': [
      bibliographyData,
      'Hall, H. K. (1957). Correlation of the Base Strengths of Amines 1. Journal of the American Chemical Society, 79(20), 5441–5444. https://doi.org/10.1021/ja01577a030',
      { lang: 'foo' }
    ],
    'custom style': [
      bibliographyData,
      'Correlation of the Base Strengths of Amines 1',
      { style: 'custom' }
    ],
    'custom locale': [
      [{}],
      '(Custom).',
      { lang: 'custom' }
    ],
    'custom underscore locale': [
      [{}],
      '(Custom).',
      { lang: 'zh_CN' }
    ],
    'style-default locale': [
      bibliographyData,
      'Hall, H.K. (1957) « Correlation of the Base Strengths of Amines 1 », Journal of the American Chemical Society, 79(20), p. 5441‑5444. Disponible sur: https://doi.org/10.1021/ja01577a030.',
      { style: 'harvard1:fr-FR' }
    ],
    'unavailable style-default locale': [
      bibliographyData,
      'Hall, H.K. (1957) “Correlation of the Base Strengths of Amines 1,” Journal of the American Chemical Society, 79(20), pp. 5441–5444. Available at: https://doi.org/10.1021/ja01577a030.',
      { style: 'harvard1:en-GB' }
    ],
    'static pre/append': [
      bibliographyData,
      'aHall, H. K. (1957). Correlation of the Base Strengths of Amines 1. Journal of the American Chemical Society, 79(20), 5441–5444. https://doi.org/10.1021/ja01577a030b',
      { prepend: 'a', append: 'b' }
    ],
    'static prepend only': [
      bibliographyData,
      'aHall, H. K. (1957). Correlation of the Base Strengths of Amines 1. Journal of the American Chemical Society, 79(20), 5441–5444. https://doi.org/10.1021/ja01577a030',
      { prepend: 'a', append: null }
    ],
    'dynamic pre/append': [
      bibliographyData,
      '20Hall, H. K. (1957). Correlation of the Base Strengths of Amines 1. Journal of the American Chemical Society, 79(20), 5441–5444. https://doi.org/10.1021/ja01577a03079',
      { prepend: ({ issue }) => issue, append: ({ volume }) => volume }
    ],
    'built-in format': [
      bibliographyData,
      '<div class="csl-bib-body">\n  <div data-csl-entry-id="Q23571040" class="csl-entry">Hall, H. K. (1957). Correlation of the Base Strengths of Amines 1. <i>Journal of the American Chemical Society</i>, <i>79</i>(20), 5441–5444. https://doi.org/10.1021/ja01577a030</div>\n</div>',
      { format: 'html' }
    ],
    'built-in format and pre/append': [
      bibliographyData,
      '<div class="csl-bib-body">\n  <div data-csl-entry-id="Q23571040" class="csl-entry">aHall, H. K. (1957). Correlation of the Base Strengths of Amines 1. <i>Journal of the American Chemical Society</i>, <i>79</i>(20), 5441–5444. https://doi.org/10.1021/ja01577a030b</div>\n</div>',
      { format: 'html', style: 'apa', prepend: 'a', append: 'b' }
    ],
    'specific entries': [
      citationData,
      'a\nb',
      { style: 'custom', entry: ['1', '2'] }
    ],
    'specific entry': [
      citationData,
      'e',
      { style: 'custom', entry: ['3'] }
    ],
    'as array': [
      citationData,
      [
        ['1', '  <div data-csl-entry-id="1" class="csl-entry"><i>a</i>. (2011).</div>\n'],
        ['2', '  <div data-csl-entry-id="2" class="csl-entry">d,  c &#38; h. (2012). <i>b</i>.</div>\n'],
        ['3', '  <div data-csl-entry-id="3" class="csl-entry">f,  g. (2013). <i>e</i>.</div>\n']
      ],
      { format: 'html', style: 'apa', asEntryArray: true }
    ],
    'with hyperlinks': [
      bibliographyData,
      '<div class="csl-bib-body">\n  <div data-csl-entry-id="Q23571040" class="csl-entry">Hall, H. K. (1957). Correlation of the Base Strengths of Amines 1. <i>Journal of the American Chemical Society</i>, <i>79</i>(20), 5441–5444. <a href="https://doi.org/10.1021/ja01577a030">https://doi.org/10.1021/ja01577a030</a></div>\n</div>',
      { format: 'html', hyperlinks: true }
    ],
    'with downgraded CSL': [
      [{ 'event-title': 'foo', title: 'bar', issued: { 'date-parts': [[2026]] }, author: [{ literal: 'baz' }], type: 'speech' }],
      'baz (2026) “bar.” foo.',
      { style: 'harvard1:unpatched', downgradeCsl: true }
    ],
    'with non-downgraded CSL': [
      [{ 'event-title': 'foo', title: 'bar', issued: { 'date-parts': [[2026]] }, author: [{ literal: 'baz' }], type: 'speech' }],
      'baz (2026) “bar.”',
      { style: 'harvard1:unpatched' }
    ]
  },
  citation: {
    simple: [citationData, '(A, 2011; d & h, 2012)', { entry: ['1', '2'] }],
    'single entry': [citationData, '(d & h, 2012)', { entry: '2' }],
    'no entry option': [citationData, '(A, 2011; d & h, 2012; f, 2013)'],
    'with locator': [citationData, '(A, 2011, p. 123)', { entry: { id: '1', locator: 123, label: 'page' } }],
    'with suffix': [citationData, '(A, 2011 [!])', { entry: { id: '1', suffix: '[!]' } }],
    'style-default locale': [citationData, '(2011; d et h, 2012; f, 2013)', { style: 'harvard1:fr-FR' }],
    'with context': [citationData, '(3)', { style: 'vancouver', citationsPre: ['1', '2'], entry: '3' }],
    'with citationItems': [citationData, '(A, 2011 [!])', { entry: { citationItems: [{ id: '1', suffix: '[!]' }], properties: { noteIndex: 0 } } }]
  }
}
